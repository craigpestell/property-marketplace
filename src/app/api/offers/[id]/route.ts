...
export const GET = withClientUid<{ id: string }>(
  async (_request: NextRequest, { clientUid, params }) => {
    try {
      const offerUid = params?.id;
...
export const PUT = withClientUid<{ id: string }>(
  async (request: NextRequest, { clientUid, email, params }) => {
    try {
      const offerUid = params?.id;
    ...
    `;
      ...
      `;
        await pool.query(counterQuery, [
          ...
          counter_amount,
          counter_terms || '',
          email, // Use email from middleware context
        ]);
      }
      ...
      const recipientClientUid =
        status === 'withdrawn'
          ? offer.seller_client_uid
          : offer.buyer_client_uid;
