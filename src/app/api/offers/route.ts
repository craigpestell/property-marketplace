...
export const POST = withClientUid(
  async (request: NextRequest, { clientUid, email }) => {
    try {
      ...
      // Use buyer's client_uid from middleware
      const buyerClientUid = clientUid;
    ...
    `;
      ...
      await pool.query(
       ...
       (user_email, title, message, type, related_offer_uid, priority, client_uid)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`,
        [
          seller_email,
          'New Offer Received',
          notificationMessage,
          'offer_received', // Changed from 'offer' to match the allowed values in the check constraint
          offer_uid,
          'high',
          property.seller_client_uid,
        ],
      );
